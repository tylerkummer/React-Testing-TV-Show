...
const mockEpisodesData = [
    {
        ...
        name:"Chapter One: The Vanishing of Will Byers","season":1,"number":1,"airdate":"2016-07-15",
        airtime:"",
...
test("Episodes renders list of episodes", () => {
    const { queryAllByTestId, rerender } = render(
        <Episodes episodes={[]} />
    );
    expect(queryAllByTestId(/episode-test/i)).toHaveLength(0);

    rerender(<Episodes episodes={mockEpisodesData} />);
    expect(queryAllByTestId(/episode-test/i)).toHaveLength(2);
});